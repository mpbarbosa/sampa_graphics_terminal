...
cargo test  --manifest-path crates/freemem/Cargo.toml     # free gauge: parse_free → Mem/Swap stats (fails safe to None)
...
- **`free` memory gauge** (`crates/freemem`, `sampa-freemem`): headless parse core for a memory gauge that **also shares `Ctrl+Shift+E`** (typed `free` → gauge; the dispatcher routes `cd`→tree, `du`→treemap, `free`→gauge, else→ps). `parse_free(output)` turns `free -k` output into `FreeInfo { mem, swap }` (KiB); fails safe to `None` without a usable `Mem:` row. Pure `std` + serde, **no shell, no Tauri**, tested against sample + real `free`. The bridge command `run_free()` runs a read-only `free -k` (instant — reads `/proc/meminfo`, no timeout) and parses it. The `#freegauge` overlay renders proportional segmented bars (used / buff-cache / free, + swap) with a legend; **purely informational — nothing is composed or run**.
