...
- [x] **Command palette** (M4 Phase 4.1, `Ctrl+Shift+P`): flexible search over `$PATH` executables (`crates/palette`) that **inserts** the command at the prompt — never auto-runs. Matching is tiered — exact ▸ prefix ▸ word-boundary ▸ substring ▸ loose subsequence — so typing `grep` surfaces the whole family (`egrep`, `git-grep`, `ast-grep`, …), and a space ANDs terms (`git grep` → `git-grep`).
...
- [x] **CI + throughput benchmarks** (M5 Phase 5.1): [`.github/workflows/ci.yml`](.github/workflows/ci.yml) runs the headless crate tests, frontend build, Tauri app build, and two release benchmarks — **OSC-scan** (~1300 MiB/s) and **PTY pump + flood stability** (~490 MiB/s, drains 64 MiB without stalling) — with numbers printed for trend tracking and a lenient regression floor. Run locally: `cargo run --release --manifest-path crates/shellint/Cargo.toml --example bench_scan`.
- [x] **VT conformance** (M5): scored against the **esctest** suite — Sampa implements **DECRQCRA** (rectangular-area checksum) so the suite can read the screen back to verify rendered contents. Baseline **305 passing** (core cursor/erase/insert-delete/scroll/SGR + screen read-back); remaining failures are query/report and VT400 features xterm.js doesn't implement. Runner + analysis in [tools/conformance/](tools/conformance/); releases gate on the pass count. A **vttest** visual smoke (cursor addressing, character sets incl. DEC line-drawing) is documented in [tools/conformance/vttest.md](tools/conformance/vttest.md).
- [x] **Escape-sequence hardening** (M5, §13): **OSC 52** clipboard writes are gated — `[clipboard] osc52_write = ask|allow|deny` (default `ask` shows a consent modal with the byte count + preview); clipboard **reads are always denied**. Window/tab titles (OSC 0/2) are **sanitized** (control chars stripped, length-capped) so an application can't inject them. **Query replies are safe:** DA/DSR/DECRQSS answer with fixed or cursor-derived values only, and the **title-report** ops (`CSI 20/21 t`) are refused — so no escape sequence can echo attacker-controlled bytes back into your shell's input.
